...
**1. The Shortcut** — generated by `shortcut/build-shortcut.mjs`, signed with
`shortcuts sign --mode anyone`, embedded in the relay and served from
`GET /sms-code-bridge.shortcut`. The setup page links to that file **directly**. Nobody
hand-builds it.

Do **not** use `shortcuts://import-shortcut?url=…`. That scheme only accepts iCloud share
links; given a self-hosted https URL it fails with "The shortcut URL provided was invalid"
without ever fetching the file. Reproduced on macOS with both encoded and unencoded forms of
the parameter, while the very same file imports cleanly when opened directly — so the file
was never the problem and the encoding never was either.

Signing requires macOS and cannot run in a Worker, so the pairing code is **not** baked in
per user: everyone gets the same signed file and supplies their code through the shortcut's
import question. That question binds by **`ActionIndex`, not UUID** — a second reason the
Text action must stay first, since reordering silently rewrites the wrong field.

Three actions, deliberately minimal
...
3. Phone opens that page (served by the same Worker, pairing code pre-embedded):
   [ Copy pairing code ]   [ Download Shortcut → /sms-code-bridge.shortcut ]
   then: tap the Safari download → opens in Shortcuts → paste code → Add
