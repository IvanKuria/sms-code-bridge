...
`shortcuts sign --mode anyone`, embedded in the relay and served from `GET /shortcut`. The
setup page links to it via `shortcuts://import-shortcut?url=…`, which hands the file straight
to the Shortcuts app. Nobody hand-builds it.
...
3. Phone opens that page (served by the same Worker, pairing code pre-embedded):
   [ Copy pairing code ]   [ Add Shortcut → shortcuts://import-shortcut ]
